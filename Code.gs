...
function submitApplication(data) {
  try {
    ...
    Logger.log('Phone: ' + data.phone); // ✨ เพิ่ม log
    Logger.log('========================================');
    ...
    const rowData = headers.map((header) => {
      ...
      switch(header) {
        ...
        case 'อีเมล':
          value = data.email || '';
          break;
          case 'เบอร์โทรศัพท์': // ✨ เพิ่มเคส phone
          value = data.phone || '';
          break;
        case 'คุณสมบัติ':
        ...
        case 'คำอธิบายหน่วยงาน':
          ...
          break;
        case '1. ทำไมถึงสนใจเรียนหลักสูตร 4ส และคาดหวังอะไรต่อหลักสูตร':
          value = data.whyInterested || '';
          break;
        case '2. ลักษณะงานที่ทำ มีความเชื่อมโยงกับหลักสูตรอย่างไร':
          value = data.workConnection || '';
          break;
        case '3. ท่านจะนำองค์ความรู้จากหลักสูตรไปประยุกต์ใช้อย่างไร':
          value = data.relevantExperience || '';
          break;
        case 'สถานะ (เจ้าหน้าที่)':
          value = 'รอพิจารณา';
          break;
        case 'สถานะ (ดร.ชลัท)':
          value = 'รอพิจารณา';
          break;
        case 'สถานะ (ดร.อภิญญา)':
          value = 'รอพิจารณา';
          break;
        case 'หมายเหตุ':
          value = '';
...
function updateStatus(data) {
  try {
    ...
    let statusColIndex;
    if (data.committeeMember === 'เจ้าหน้าที่') {
      statusColIndex = colMap['สถานะ (เจ้าหน้าที่)'];
    } else if (data.committeeMember === 'ดร.ชลัท') {
      statusColIndex = colMap['สถานะ (ดร.ชลัท)'];
    } else if (data.committeeMember === 'ดร.อภิญญา') {
      statusColIndex = colMap['สถานะ (ดร.อภิญญา)'];
    }
...
function sendConfirmationEmail(email, anonymousId) {
  ...
  const htmlBody = `
    <div style="font-family: 'Sarabun', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <div style="background: #f8fafc; padding: 30px; border-radius: 0 0 10px 10px;">
        <p style="font-size: 16px; color: #334155;">เรียน ผู้แจ้งความประสงค์สมัครเข้าศึกษาอบรม</p>
        
        <p style="font-size: 16px; color: #334155; line-height: 1.6;">
          ขอขอบคุณที่ท่านได้แจ้งความประสงค์สมัครเข้าศึกษาอบรม<br>
          <strong>หลักสูตรประกาศนียบัตรชั้นสูงการเสริมสร้างสังคมสันติสุข รุ่นที่ 16</strong>
        </p>
      
        
        <p style="font-size: 16px; color: #334155; line-height: 1.6;">
          หลักสูตรจะแจ้งผลการพิจารณารอบแรกให้ท่านทราบทางอีเมลนี้ <strong>ในวันจันทร์ที่ 9 มีนาคม 2569</strong><br>
          <strong>หากท่านผ่านการพิจารณารอบแรก ท่านจะได้รับลิงก์ใบสมัคร เพื่อดำเนินการตามขั้นตอนต่อไป
        </p>
  ...
  const plainBody = `
เรียน ผู้แจ้งความประสงค์สมัครเข้าศึกษาอบรม 

ขอขอบคุณที่ท่านได้แจ้งความประสงค์สมัครเข้าศึกษาอบรมหลักสูตรประกาศนียบัตรชั้นสูงการเสริมสร้างสังคมสันติสุข
รุ่นที่ 16

หลักสูตรจะแจ้งผลการพิจารณารอบแรกให้ท่านทราบทางอีเมลนี้ ในวันจันทร์ที่ 9 มีนาคม 2569
หากท่านผ่านการพิจารณารอบแรก ท่านจะได้รับลิงก์ใบสมัคร เพื่อดำเนินการตามขั้นตอนต่อไป
...
function testSendEmail() {
  const testEmail = "webmaster@kpi.ac.th"; // ใช้อีเมลจริงของคุณ
...
function testCheckDuplicateManually() {
  ...
  const testEmail = 'webmaster@kpi.ac.th'; // เปลี่ยนเป็นอีเมลที่มีอยู่ในชีท
...
function testCheckDuplicateDebug() {
  ...
  const testEmail = 'webmaster@kpi.ac.th'; // เปลี่ยนเป็นอีเมลที่มีจริง
